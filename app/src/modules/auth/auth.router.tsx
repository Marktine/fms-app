...
export class AuthRouter {
  ...
  private registerRoutes() {
    ...
    this.router.post(
      '/login',
      zValidator('form', loginSchema, (result, _c) => {
        if (!result.success) {
    ...
    this.router.post(
      '/register',
      zValidator('form', registerSchema, (result, _c) => {
        if (!result.success) {
      ...
      async (c) => {
        const { email, password } = c.req.valid('form');
        const { errorMessage, result: createUserResult } = await this.authService.register(
          email,
          password,
        );
        if (createUserResult) {
